...
function delegate(target, fn) {
    ...
    let arrButton = target.getElementsByTagName('button');

    for ( var i = 0; i < arrButton.length; i++) {
        arrButton[i].addEventListener('click', fn);
    }
...
function once(target, fn) {
    ...
    let intFn = function() {
        fn();
        target.removeEventListener('click', intFn);
    }

    target.addEventListener('click', intFn);
